...
public class StoredProcedureConfig implements MetadataBuilderContributor {
    ...
    public void contribute(MetadataBuilder metadataBuilder) {
        ...
        metadataBuilder.applySqlFunction(
                "ACCEPT_ORDER",
                new StandardSQLFunction("accept_order")
        );

        metadataBuilder.applySqlFunction(
                "CANCELLED_ORDER",
                new StandardSQLFunction("cancelled_order")
        );

        metadataBuilder.applySqlFunction(
                "UPDATE_ORDER_STATUS",
                new StandardSQLFunction("update_order_status")
        );

        metadataBuilder.applySqlFunction(
                "GET_PAYMENT_HISTORY",
                        new StandardSQLFunction("get_payment_history")
        );
    }
    @Configuration
